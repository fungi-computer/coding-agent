...
export function extensionForImageMimeType(mimeType: string): null | string {
 ...
 */
async function convertToPng(bytes: Uint8Array): Promise<null | Uint8Array> {
...
function selectPreferredImageMimeType(mimeTypes: string[]): null | string {
  const normalized = mimeTypes
    .map((t) => t.trim())
    .filter(Boolean)
    .map((t) => ({ base: baseMimeType(t), raw: t }));
