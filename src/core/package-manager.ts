...
export type MissingSourceAction = "error" | "install" | "skip";
...
type ParsedSource = GitSource | LocalSource | NpmSource;
...
type SourceScope = "project" | "temporary" | "user";
...
type SkillDiscoveryMode = "agents" | "pi";
...
export class DefaultPackageManager implements PackageManager {
  ...
  async checkForAvailableUpdates(): Promise<PackageUpdate[]> {
    ...
    const allPackages: { pkg: PackageSource; scope: SourceScope }[] = [];
    ...
    const checks = packageSources
      ...
      .map((entry) => async (): Promise<PackageUpdate | undefined> => {
        ...
        if (parsed.type === "npm") {
          ...
          const hasUpdate = await this.npmHasAvailableUpdate(
            parsed,
            installedPath,
          );
          if (!hasUpdate) {
    ...
    return results.filter(
      (result): result is PackageUpdate => result !== undefined,
    );
  }
  ...
  private addAutoDiscoveredResources(
    ...
    const projectAgentsSkillDirs = collectAncestorAgentsSkillDirs(
      this.cwd,
    ...
    addResources(
      ...
      [
        ...
        ...projectAgentsSkillDirs.flatMap((dir) =>
          collectAutoSkillEntries(dir, "agents"),
        ),
      ],
      projectMetadata,
  ...
  private buildNoMatchingPackageMessage(
    ...
    const suggestion = this.findSuggestedConfiguredSource(
      source,
      configuredPackages,
    );
    if (!suggestion) {
  ...
  private dedupePackages(
    packages: { pkg: PackageSource; scope: SourceScope }[],
  ): { pkg: PackageSource; scope: SourceScope }[] {
  ...
  private findSuggestedConfiguredSource(
    ...
    for (const pkg of configuredPackages) {
      ...
      if (parsed.type === "git") {
        ...
        const shorthandWithRef = parsed.ref
          ? `${shorthand}@${parsed.ref}`
          : undefined;
        if (
  ...
  private async getGitUpstreamRef(
    installedPath: string,
  ...
  private async getLocalGitUpdateTarget(
    ...
    try {
      ...
      const head = await this.runCommandCapture(
        "git",
        ["rev-parse", "@{upstream}"],
        {
          cwd: installedPath,
  ...
  private async gitHasAvailableUpdate(installedPath: string): Promise<boolean> {
    ...
    try {
      const localHead = await this.runCommandCapture(
        "git",
        ["rev-parse", "HEAD"],
        {
          cwd: installedPath,
  ...
  private readPiManifest(packageRoot: string): null | PiManifest {
  ...
  private resolveLocalEntries(
    ...
    const { patterns, plain } = splitPatterns(entries);
  ...
  private async resolvePackageSources(
    sources: { pkg: PackageSource; scope: SourceScope }[],
    accumulator: ResourceAccumulator,
    onMissing?: (source: string) => Promise<MissingSourceAction>,
  ): Promise<void> {
    for (const { pkg, scope } of sources) {
      ...
      if (parsed.type === "npm") {
        ...
        const needsInstall =
          ...
          (parsed.pinned &&
            !(await this.installedNpmMatchesPinnedVersion(
              parsed,
              installedPath,
            )));
        if (needsInstall) {
  ...
  private runCommandSync(command: string, args: string[]): string {
    ...
    if (result.status !== 0) {
      throw new Error(
        `Failed to run ${command} ${args.join(" ")}: ${result.stderr || result.stdout}`,
      );
    }
  ...
  private runNpmCommandSync(args: string[]): string {
    const npmCommand = this.getNpmCommand();
    return this.runCommandSync(npmCommand.command, [
      ...npmCommand.args,
      ...args,
    ]);
  }
  ...
  private toResolvedPaths(accumulator: ResourceAccumulator): ResolvedPaths {
    const toResolved = (
      ...
      resolved.sort(
        (a, b) =>
          resourcePrecedenceRank(a.metadata) -
          resourcePrecedenceRank(b.metadata),
      );
      return resolved;
    };
  ...
  private async updateGit(
    ...
    const refreshedTargetHead = await this.runCommandCapture(
      "git",
      ["rev-parse", target.ref],
      {
        cwd: targetDir,
        timeoutMs: NETWORK_TIMEOUT_MS,
      },
  ...
  private async withProgress(
    ...
    this.emitProgress({ action, message, source, type: "start" });
    try {
      await operation();
      this.emitProgress({ action, source, type: "complete" });
    } catch (error) {
      const errorMessage =
...
function findGitRepoRoot(startDir: string): null | string {
...
function readPiManifestFile(packageJsonPath: string): null | PiManifest {
  try {
    const content = readFileSync(packageJsonPath, "utf-8");
    const pkg = JSON.parse(content) as { pi?: PiManifest };
    return pkg.pi ?? null;
...
function resolveExtensionEntries(dir: string): null | string[] {
