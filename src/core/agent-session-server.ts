...
export class AgentSessionServer {
	...
	private readonly _globalSubscribers: Set<(event: GlobalServerEvent) => void> = new Set();
	...
	async createSession(cwd: string): Promise<{ sessionId: string; snapshot: SessionSnapshot }> {
		const { sessionId } = await this._sessionStore.createSession(cwd);

		const session = await this._sessionFactory.createSession({ sessionId, cwd });
		...
		this._emitGlobal({
			...
			sessionId,
			info: data ? this._toListItem(data) : { id: sessionId, cwd, createdAt: Date.now(), modifiedAt: Date.now(), messageCount: 0 },
		});
	...
	async joinSession(sessionId: string): Promise<SessionSnapshot> {
		...
		if (!state) {
			...
			const session = await this._sessionFactory.createSession({ sessionId, cwd: data.cwd });
	...
	async command(sessionId: string, cmd: SessionCommand): Promise<void> {
		...
		switch (cmd.type) {
			...
			case "abort":
				...
				break;
			case "set_model": {
				const resolvedModel = this._modelRegistry.find(cmd.provider ?? "", cmd.modelId);
				if (!resolvedModel) {
	...
	subscribeSession(sessionId: string, listener: (event: AgentSessionSyncEvent) => void): () => void {
		const state = this._sessions.get(sessionId);
		if (!state) {
			throw new Error(`Session not found: ${sessionId}`);
		}
