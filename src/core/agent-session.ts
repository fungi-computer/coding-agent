...
class ConsoleLogger implements AgentLogger {
	...
	error(event: string, data?: Record<string, unknown>, error?: Error) {
		console.error(`[ERROR] ${event}`, error?.stack ?? "", data ? JSON.stringify(data) : "");
	}
}
...
export class AgentSession {
	...
	private _emit(event: AgentSessionEvent): void {
		this._logger.debug("event_emitted", { type: event.type, listenerCount: this._eventListeners.length });
		if (event.type === "message_start" || event.type === "message_update" || event.type === "message_end") {
			if (event.type === "message_update") {
				...
				const enrichedEvent = event.type === "message_start"
					? { type: "message_start" as const, message: event.message, id: event.id }
					: { type: "message_end" as const, message: event.message, id: event.id };
				for (const l of this._eventListeners) {
					l(enrichedEvent);
				}
			}
	...
	private _handleAgentEvent = (event: AgentEvent): void => {
		this._logger.debug("agent_event", { type: event.type });
	...
	setActiveToolsByName(toolNames: string[]): void {
		...
		const hasChanged = currentToolNames.length !== newToolNames.length ||
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		...
		this._logger.debug("model_check", { provider: this.model.provider, id: this.model.id });
		...
		try {
			...
			this._logger.debug("prompt_complete");
		} catch (e: any) {
	...
	async sendCustomMessage<T = unknown>(
		...
		if (options?.deliverAs === "nextTurn") {
			...
			this._emit({ type: "message_start", message: appMessage, id: nanoid() });
			this._emit({ type: "message_end", message: appMessage, id: nanoid() });
		}
	}
	...
	private _buildRuntime(options: {
		...
		this._logger.info("build_runtime_start", { toolsCount: this._tools.length });

		const extensionsResult = this._resourceLoader.getExtensions();
		this._logger.debug("build_runtime_extensions", { extensionsCount: extensionsResult.extensions.length });
		...
		this._logger.debug("build_runtime_has_extensions_tools", { hasExtensions, hasTools });
		...
		this._extensionRunner =
			hasExtensions || hasTools
				...
				: undefined;
		this._logger.debug("build_runtime_extension_runner", { created: !!this._extensionRunner });
